...
module.exports = class PlayCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      memberName: 'play',
      group: 'music',
      description: 'Coloca Una Cancion De Youtube',
      guildOnly: true,
  ...
  async run(message, { query }) {
    ...
    if (!voiceChannel) return message.say('Entra A Un Canal De Voz Y Vuelve A Intentarlo');
    // end initial check
    ...
    if (query.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
      ...
      try {
        ...
        console.error(err);
        return message.say('Ha Ocurrido Un Error, Por Favor Intentelo Más Tarde');
      }
    }
    try {
      ...
      try {
        ...
        return message.say(
          'Por Favor Escribe Un Numero Del 1 Al 5 o Exit'
        );
      }
      ...
      return message.say(
        'Hubo Un Error Buscando La Cancion Que Pediste'
      );
    }
  }
      ...
      .then(connection => {
        const dispatcher = connection
          ...
          .on('start', () => {
            ...
            if (queue[1]) videoEmbed.addField('Proxima Cancion:', queue[1].title);
